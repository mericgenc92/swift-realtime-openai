public struct Response: Identifiable, Equatable, Hashable, Codable, Sendable {
	...
	/// Details accompanying a non-`completed` status. For a `failed` response
	/// this carries the actual error — e.g. `rate_limit_exceeded` with a
	/// "try again in X.XXs" message. Without decoding this, a failed response
	/// is indistinguishable from a successful empty one.
	public struct StatusDetails: Equatable, Hashable, Codable, Sendable {
		public struct StatusError: Equatable, Hashable, Codable, Sendable {
			/// The type of error (e.g. "server_error").
		...
		/// Mirrors the response status ("failed", "incomplete", "cancelled").
		public let type: String?

		/// Why the response is incomplete/cancelled, if applicable.
		public let reason: String?

		/// The error that caused a failed response, if any.
		public let error: StatusError?
	}

	public struct Usage: Equatable, Hashable, Codable, Sendable {
		// All fields optional: OpenAI Realtime API may return usage in different formats
		// (e.g., duration-based {type: "duration", seconds: N} instead of token-based).
	...
	/// Additional details when the status is not `completed`.
